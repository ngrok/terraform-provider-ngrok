...
func resourceAgentIngressesCreate(d *schema.ResourceData, m interface{}) (err error) {
	...
	if v, ok := d.GetOk("certificate_management_policy"); ok {
		arg.CertificateManagementPolicy = expandAgentIngressCertPolicy(v)
	}
...
func resourceAgentIngressesGetDecode(d *schema.ResourceData, res *restapi.AgentIngress, resp *http.Response, err error) error {
	...
	case err != nil:
		...
		return err
	default:
		d.Set("certificate_management_policy", flattenAgentIngressCertPolicy(res.CertificateManagementPolicy))
